...
from util.icbhi_dataset import ICBHIDataset_train, ICBHIDataset_val
...
def set_loader(args):
    if args.dataset == 'icbhi':
        ...
        train_dataset = ICBHIDataset_train(train_flag=True, transform=train_transform, args=args, print_flag=True)
        val_dataset = ICBHIDataset_val(train_flag=False, transform=val_transform, args=args, print_flag=True)
...
def main():
    ...
    update_json('%s' % args.model_name, best_acc, path=os.path.join(args.save_dir, 'hospital_iphone_4.json'))
